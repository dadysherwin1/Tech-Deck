# configs
path = 'C:/Users/24562353/Desktop/LightingTool/'
...
def importSequenceLighting(buttonInfo):
    getTextFields()
    cmds.file(path + sequenceName + "/sequence_lighting.mb", i=True)
    
def importShotLighting(buttonInfo):
    getTextFields()
    cmds.file(path + sequenceName + "/" + shotName + ".mb", i=True)
...
cmds.setParent("..")
cmds.separator(h=30, style="none")
cmds.text(label="Import lights")
cmds.separator(h=10, style="none")
cmds.rowLayout(numberOfColumns=2)
cmds.button(label="Import Sequence", command=importSequenceLighting, width=200)
cmds.button(label="Import Shot", command=importShotLighting, width=200)
